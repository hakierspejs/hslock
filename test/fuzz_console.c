 ...
 * The console command logic branches on RTC state (clock_get_unix_time()
 * returning false → boot-bypass window), boot uptime (time_us_64() vs
 * BOOT_BYPASS_WINDOW_US),
...
typedef struct {
    uint32_t clock_unix;   /* clock_get_unix_time() out value, when clock_set below */
    ...
    bool     clock_set;    /* clock_get_unix_time() return value: false == RTC not set */
} fuzz_env_t;

static fuzz_env_t g_env = {1700000000u, 1234567890ULL, false, false, false, 0, true};
...
bool clock_get_unix_time(uint32_t *out_unix_time) {
    if (!g_env.clock_set)
        return false;
    *out_unix_time = g_env.clock_unix;
    return true;
}
...
static const fuzz_env_t ENVS[] = {
    {1700000000u, 1234567890ULL, false, false, false, 0, true},
    {0u, 1000ULL, false, false, false, 0, false},
    {0u, 400000000ULL, false, false, false, 0, false},
    {1700000000u, 1234567890ULL, true, true, true, 1700000000u, true},
    {1700000000u, 1234567890ULL, false, true, false, 0, true},
};
