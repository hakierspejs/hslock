...
int backup_export(uint8_t *buf, size_t buf_size) {
    ...
    size_t needed = sizeof(backup_header_t)
                  + count * sizeof(backup_key_t);
...
bool backup_import(const uint8_t *buf, size_t size) {
    ...
    size_t expected = sizeof(backup_header_t)
                    + hdr->key_count * sizeof(backup_key_t);
    if (size < expected) {
        printf("[backup] import: truncated data\r\n");
        return false;
    }

    const backup_key_t *keys =
        (const backup_key_t *)(buf + sizeof(backup_header_t));
